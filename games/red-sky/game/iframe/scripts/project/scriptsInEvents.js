...
const scriptsInEvents = {
	...
	async E_game_Event43_Act2(runtime, localVars)
	{
		const save = runtime.objects.save.getFirstInstance().getJsonDataCopy();
	...
	async E_game_Event46_Act2(runtime, localVars)
	{
		clearGhostData();
	...
	async E_game_Event68(runtime, localVars)
	{
	...
	async E_game_Event70(runtime, localVars)
	{
	...
	async E_game_Event77(runtime, localVars)
	{
		const camPos = Vec3.fromArray(runtime.objects.camera.getCameraPosition());
	...
	async E_game_Event81_Act1(runtime, localVars)
	{
	...
	async E_game_Event138_Act1(runtime, localVars)
	{
	...
	async E_game_Event147_Act1(runtime, localVars)
	{
	...
	async E_remoteplayer_Event32_Act4(runtime, localVars)
	{
		...
		for (const [key, value] of Object.entries(players)) {
			...
			for (const remoteInst of remotePlayers) {
				...
				remoteInst.angleDegrees = value.angle;
				remoteInst.instVars.vx = value.vx;
				remoteInst.instVars.vy = value.vy;
				remoteInst.instVars.vz = value.vz;
				remoteInst.instVars.cameraX = value.cameraX;
				remoteInst.instVars.cameraY = value.cameraY;
				remoteInst.instVars.cameraZ = value.cameraZ;
				remoteInst.instVars.inState = 1;
				remoteInst.instVars.state = value.state;
				remoteInst.instVars.animation = value.animation;
				remoteInst.instVars.upX = value.upX;
				remoteInst.instVars.upY = value.upY;
				remoteInst.instVars.upZ = value.upZ;
				remoteInst.instVars.forwardX = value.forwardX;
				remoteInst.instVars.forwardY = value.forwardY;
				remoteInst.instVars.forwardZ = value.forwardZ;
				//remoteInst.instVars.accessory = value.accessory;
		
				/*let {x,y,z} = Utils.computeEulerRotation(localVars.forwardX, localVars.forwardY, localVars.forwardZ, localVars.upX, localVars.upY, localVars.upZ)
		
				localVars.rx = x;
				localVars.ry = y;
				localVars.rz = z;*/
				if (remoteInst.instVars.skin != value.skin) {
					remoteInst.instVars.skin = value.skin;
					remoteInst.signal("updateSkin");	
				}
				
				if (value.emote != "") {
					runtime.callFunction("createEmoteAtUID", value.emote, remoteInst.uid)
				}
		
				if (remoteInst.instVars.playerName != value.name) {
					runtime.callFunction("updateRemotePlayerName", value.name, remoteInst.uid)
				}
	...
	async E_remoteplayer_Event40_Act1(runtime, localVars)
	{
		const camPos = Vec3.fromArray(runtime.objects.camera.getCameraPosition());
	...
	async E_remoteplayer_Event49_Act1(runtime, localVars)
	{
		...
		remoteInst.instVars.cameraX = player.x;
		remoteInst.instVars.cameraY = player.y;
		remoteInst.instVars.cameraZ = player.zElevation + 128;
		remoteInst.instVars.inState = true;
		remoteInst.instVars.state = "";
		remoteInst.instVars.animation = player.instVars.state;
		remoteInst.instVars.upX = camera.upX;
		remoteInst.instVars.upY = camera.upY;
		remoteInst.instVars.upZ = camera.upZ;
		remoteInst.instVars.forwardX = Math.cos(player.angle);
		remoteInst.instVars.forwardY = Math.sin(player.angle);
		remoteInst.instVars.forwardZ = 1;
		//remoteInst.instVars.accessory = camera.accessory;
	},

	async E_remoteplayer_Event109(runtime, localVars)
	{
		const camPos = Vec3.fromArray(runtime.objects.camera.getCameraPosition());
		const camLook = Vec3.fromArray(runtime.objects.camera.getLookVector());
	...
	async E_remoteplayer_Event111_Act1(runtime, localVars)
	{
		const camPos = Vec3.fromArray(runtime.objects.camera.getCameraPosition());
		...
		for(const inst of runtime.objects.remoteCharacter.getAllInstances()){
			...
			if (dist < threshold) {
				...
				inst.effects[0].setParameter(2, (dist - 140) / 40);		
			}
			else {
	...
	async E_skin_Event70(runtime, localVars)
	{
		const player = runtime.objects.player.getFirstInstance();
