...
'use strict';
{
    ...
    ;
    const WEBVIEW_EXPORT_TYPES = new Set(["cordova", "playable-ad-single-file", "playable-ad-zip", "instant-games"]);
    ...
    window.RuntimeInterface = class RuntimeInterface {
        ...
        async _Init(opts) {
            ...
            if (this._exportType === "nwjs" && self["nw"] && self["nw"]["App"]["manifest"]["c3-steam-mode"]) {
                ...
                this._AddRAFCallback( () => {
                    frameNum++;
                    document.documentElement.style.opacity = frameNum % 2 === 0 ? "1" : "0.999"
                }
                )
            }
        ...
        async _InitDOM(opts, port2) {
            ...
            this._runtimeDomHandler._AddDefaultCanvasEventHandlers(canvas);
            this._runtimeDomHandler._AddDefaultHTMLWrapEventHandlers(htmlWrap);
            const runtimeScriptList = await Promise.all(opts.runtimeScriptList.map(url => this._MaybeGetPlatformSpecificScriptURL(url)));
            ...
            if (projectMainScriptPath)
                try {
                    ...
                    alert(`Failed to load the project main script (${projectMainScriptPath}). Check all your JavaScript code has valid syntax, all imports are written correctly, and that an exception was not thrown running the script. Press F12 and check the console for error details.`)
                }
        ...
        GetCssDisplayMode() {
            ...
            if (standaloneExportTypes.has(exportType))
                return "standalone";
        ...
        _UpdateHTMLElementsZOrder() {
            if (this._pendingUpdateHTMLZOrder)
                return;
        ...
        _DoUpdateHTMLElementsZOrder() {
            ...
            for (const domHandler of this._domHandlers)
                if (domHandler instanceof window.DOMElementHandler) {
                    ...
                    if (elemStates)
                        allElementStates.push(...elemStates)
                }
        ...
        _DoUpdateHTMLElementsZOrderOnHTMLLayer(htmlIndex, arr) {
            if (arr.length <= 1)
                return;
            if (htmlIndex >= this._canvasLayers.length)
                return;
            ...
            for (; i < len; ++i)
                if (newChildren[i] !== existingChildren[i])
                    break;
            let j = i;
            for (; j < len; ++j)
        ...
        async _MaybeGetPlatformSpecificScriptURL(url) {
            if (this._exportType === "cordova" && (url.startsWith("file:") || this._isFileProtocol && this.IsRelativeURL(url))) {
                ...
                if (this._localFileStrings.hasOwnProperty(url))
                    return {
                        isStringSrc: true,
                        str: this._localFileStrings[url]
                    };
        ...
        _OnWrapperMessage(msg, additionalObjects) {
            if (msg === "entered-fullscreen") {
                ...
                if (type === "directory-handles")
                    ...
                    this.PostToRuntimeComponent("runtime", "wrapper-extension-message", msg);
                else
...
'use strict';
{
    ...
    function PreventDefaultOnHTMLWrap(e) {
        if (!e.target.tagName)
            return;
        if (e.target.classList.contains("c3htmlwrap"))
            e.preventDefault()
    }
    ...
    const HANDLER_CLASS = class RuntimeDOMHandler extends self.DOMHandler {
        ...
        Attach() {
            ...
            window.addEventListener("mouseup", e => this._OnMouseEvent("mouseup", e, DISPATCH_SCRIPT_ONLY));
        ...
        _OnVisibilityChange(isHidden) {
            if (this._pageVisibilityIsHidden === isHidden)
                return;
...
'use strict';
{
    ...
    self.JobSchedulerDOM = class JobSchedulerDOM {
        ...
        async Init() {
            ...
            const dispatchWorkerScriptUrl = this._runtimeInterface.GetScriptFolder() + DISPATCH_WORKER_SCRIPT_NAME;
        ...
        async _CreateJobWorker() {
            ...
            const jobWorkerScriptUrl = this._runtimeInterface.GetScriptFolder() + JOB_WORKER_SCRIPT_NAME;
...
'use strict';
{
    self.C3WebAudioBuffer = class C3WebAudioBuffer extends self.C3AudioBuffer {
        ...
        async _Fetch() {
            ...
            else if (iRuntime.GetExportType() === "playable-ad-single-file" && iRuntime.IsRelativeURL(this._url)) {
    ...
    const HANDLER_CLASS = class LocalStorageDOMHandler extends self.DOMHandler {
        ...
        async _Init() {
            ...
            try {
                ...
                isPersistent = false;
                console.warn("[Construct] Error checking storage persisted state: ", err)
            }
            return {
